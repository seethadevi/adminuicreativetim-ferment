...
export class VendorListComponent implements OnInit, OnDestroy {
  ...
  constructor(private appService: AppService, private vendorService: VendorService,
  ...
  reloadGridSearch(curparams) {
    ...
    this.vendorService.getVendorsWithPageData(curparams, '')
      .subscribe(
      (response: any) => {
  ...
  // getVendorList() {
  //   this.store.dispatch({type: appAction.SHOWHIDE_APP_LOADING, payload: { flag : true}});
  //   this.vendorService.getVendorsWithPageData(this.defaultParams, '')
  //   .subscribe(
  //   (response: any) => {
  //     this.store.dispatch({type: appAction.SHOWHIDE_APP_LOADING, payload: { flag : false}});
  //     if (response.status === 'success') {
  //       this.vendors = response.res.docs;
  //     } else {
  //       this.appToastrService.showError(response.msg);
  //     }
  //   },
  ...
  //   this.appService.gotoURL('/reference/vendor/update/' + vendorId);
  // }

  createnew() {
    this.appService.gotoURL('/reference/vendor/new');
  }
